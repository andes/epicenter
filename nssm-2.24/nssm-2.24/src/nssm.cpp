...
int str_equiv(const TCHAR *a, const TCHAR *b) ***REMOVED***
...
int str_number(const TCHAR *string, unsigned long *number, TCHAR **bogus) ***REMOVED***
  if (! string) return 1;
...
int str_number(const TCHAR *string, unsigned long *number) ***REMOVED***
  TCHAR *bogus;
  return str_number(string, number, &bogus);
***REMOVED***

/* Remove basename of a path. */
void strip_basename(TCHAR *buffer) ***REMOVED***
...
static int elevate(int argc, TCHAR **argv, unsigned long message) ***REMOVED***
...
int _tmain(int argc, TCHAR **argv) ***REMOVED***
  check_console();
  ...
  if (argc > 1) ***REMOVED***
    ...
    */
    ...
    if (str_equiv(argv[1], _T("restart"))) ***REMOVED***
...
***REMOVED***
    ...
    if (str_equiv(argv[1], _T("install"))) ***REMOVED***
      if (! is_admin) exit(elevate(argc, argv, NSSM_MESSAGE_NOT_ADMINISTRATOR_CANNOT_INSTALL));
      exit(pre_install_service(argc - 2, argv + 2));
***REMOVED***
    if (str_equiv(argv[1], _T("edit")) || str_equiv(argv[1], _T("get")) || str_equiv(argv[1], _T("set")) || str_equiv(argv[1], _T("reset")) || str_equiv(argv[1], _T("unset"))) ***REMOVED***
      ...
      exit(ret);
***REMOVED***
    if (str_equiv(argv[1], _T("remove"))) ***REMOVED***
...
***REMOVED***
  ...
  */
  if (! GetStdHandle(STD_INPUT_HANDLE)) ***REMOVED***
    /* Start service magic */
    SERVICE_TABLE_ENTRY table[] = ***REMOVED*** ***REMOVED*** NSSM, service_main ***REMOVED***, ***REMOVED*** 0, 0 ***REMOVED*** ***REMOVED***;
    if (! StartServiceCtrlDispatcher(table)) ***REMOVED***
