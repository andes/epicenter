...
static HANDLE create_logging_thread(TCHAR *service_name, TCHAR *path, unsigned long sharing, unsigned long disposition, unsigned long flags, HANDLE *read_handle_ptr, HANDLE *pipe_handle_ptr, HANDLE *write_handle_ptr, unsigned long rotate_bytes_low, unsigned long rotate_bytes_high, unsigned long *tid_ptr, unsigned long *rotate_online) ***REMOVED***
  *tid_ptr = 0;

  /* Pipe between application's stdout/stderr and our logging handle. */
  if (read_handle_ptr && ! *read_handle_ptr) ***REMOVED***
    if (pipe_handle_ptr && ! *pipe_handle_ptr) ***REMOVED***
      if (CreatePipe(read_handle_ptr, pipe_handle_ptr, 0, 0)) ***REMOVED***
...
static inline unsigned long guess_charsize(void *address, unsigned long bufsize) ***REMOVED***
  if (IsTextUnicode(address, bufsize, 0)) return (unsigned long) sizeof(wchar_t);
  else return (unsigned long) sizeof(char);
***REMOVED***

static inline void write_bom(logger_t *logger, unsigned long *out) ***REMOVED***
  wchar_t bom = L'\ufeff';
  if (! WriteFile(logger->write_handle, (void *) &bom, sizeof(bom), out, 0)) ***REMOVED***
...
int get_createfile_parameters(HKEY key, TCHAR *prefix, TCHAR *path, unsigned long *sharing, unsigned long default_sharing, unsigned long *disposition, unsigned long default_disposition, unsigned long *flags, unsigned long default_flags) ***REMOVED***
  TCHAR value[NSSM_STDIO_LENGTH];

  /* Path. */
  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s"), prefix) < 0) ***REMOVED***
    log_event(EVENTLOG_ERROR_TYPE, NSSM_EVENT_OUT_OF_MEMORY, prefix, _T("get_createfile_parameters()"), 0);
    return 1;
  ***REMOVED***
  switch (expand_parameter(key, value, path, PATH_LENGTH, true, false)) ***REMOVED***
    case 0: if (! path[0]) return 0; break; /* OK. */
    default: return 2; /* Error. */
  ***REMOVED***

  /* ShareMode. */
  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s%s"), prefix, NSSM_REG_STDIO_SHARING) < 0) ***REMOVED***
    log_event(EVENTLOG_ERROR_TYPE, NSSM_EVENT_OUT_OF_MEMORY, NSSM_REG_STDIO_SHARING, _T("get_createfile_parameters()"), 0);
    return 3;
  ***REMOVED***
  switch (get_number(key, value, sharing, false)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  /* CreationDisposition. */
  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s%s"), prefix, NSSM_REG_STDIO_DISPOSITION) < 0) ***REMOVED***
    log_event(EVENTLOG_ERROR_TYPE, NSSM_EVENT_OUT_OF_MEMORY, NSSM_REG_STDIO_DISPOSITION, _T("get_createfile_parameters()"), 0);
    return 5;
  ***REMOVED***
  switch (get_number(key, value, disposition, false)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  /* Flags. */
  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s%s"), prefix, NSSM_REG_STDIO_FLAGS) < 0) ***REMOVED***
    log_event(EVENTLOG_ERROR_TYPE, NSSM_EVENT_OUT_OF_MEMORY, NSSM_REG_STDIO_FLAGS, _T("get_createfile_parameters()"), 0);
    return 7;
  ***REMOVED***
  switch (get_number(key, value, flags, false)) ***REMOVED***
...
int set_createfile_parameter(HKEY key, TCHAR *prefix, TCHAR *suffix, unsigned long number) ***REMOVED***
  TCHAR value[NSSM_STDIO_LENGTH];

  if (_sntprintf_s(value, _countof(value), _TRUNCATE, _T("%s%s"), prefix, suffix) < 0) ***REMOVED***
...
int delete_createfile_parameter(HKEY key, TCHAR *prefix, TCHAR *suffix) ***REMOVED***
  TCHAR value[NSSM_STDIO_LENGTH];
...
HANDLE write_to_file(TCHAR *path, unsigned long sharing, SECURITY_ATTRIBUTES *attributes, unsigned long disposition, unsigned long flags) ***REMOVED***
...
static void rotated_filename(TCHAR *path, TCHAR *rotated, unsigned long rotated_len, SYSTEMTIME *st) ***REMOVED***
  if (! st) ***REMOVED***
    SYSTEMTIME now;
    st = &now;
    GetSystemTime(st);
...
void rotate_file(TCHAR *service_name, TCHAR *path, unsigned long seconds, unsigned long low, unsigned long high) ***REMOVED***
  unsigned long error;
  ...
  if (file) ***REMOVED***
    /* Get file attributes. */
    if (! GetFileInformationByHandle(file, &info)) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
  ...
  /* Rotate. */
  if (MoveFile(path, rotated)) ***REMOVED***
...
int get_output_handles(nssm_service_t *service, STARTUPINFO *si) ***REMOVED***
  if (! si) return 1;
  ...
  /* stdin */
  if (service->stdin_path[0]) ***REMOVED***
...
***REMOVED***
  ***REMOVED***
  ...
  /* stdout */
  if (service->stdout_path[0]) ***REMOVED***
    ...
    if (service->rotate_files && service->rotate_stdout_online) ***REMOVED***
      ...
      if (! service->stdout_thread) ***REMOVED***
...
***REMOVED***
    ...
    if (! service->stdout_thread) ***REMOVED***
      if (! DuplicateHandle(GetCurrentProcess(), stdout_handle, GetCurrentProcess(), &si->hStdOutput, 0, true, DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS)) ***REMOVED***
...
***REMOVED***
  ***REMOVED***
  ...
  /* stderr */
  if (service->stderr_path[0]) ***REMOVED***
    /* Same as stdout? */
    if (str_equiv(service->stderr_path, service->stdout_path)) ***REMOVED***
      ...
      if (! DuplicateHandle(GetCurrentProcess(), si->hStdOutput, GetCurrentProcess(), &si->hStdError, 0, true, DUPLICATE_SAME_ACCESS)) ***REMOVED***
...
***REMOVED***
      ...
      if (service->rotate_files && service->rotate_stderr_online) ***REMOVED***
        ...
        if (! service->stderr_thread) ***REMOVED***
  ...
  ***REMOVED***
      ...
      if (! service->stderr_thread) ***REMOVED***
        if (! DuplicateHandle(GetCurrentProcess(), stderr_handle, GetCurrentProcess(), &si->hStdError, 0, true, DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS)) ***REMOVED***
...
***REMOVED***
  ...
  */
  ...
  if (! si->hStdInput) ***REMOVED***
    if (! DuplicateHandle(GetCurrentProcess(), GetStdHandle(STD_INPUT_HANDLE), GetCurrentProcess(), &si->hStdInput, 0, true, DUPLICATE_SAME_ACCESS)) ***REMOVED***
      log_event(EVENTLOG_ERROR_TYPE, NSSM_EVENT_DUPLICATEHANDLE_FAILED, _T("STD_INPUT_HANDLE"), _T("stdin"), error_string(GetLastError()), 0);
      return 8;
***REMOVED***
  ***REMOVED***
  if (! si->hStdOutput) ***REMOVED***
    if (! DuplicateHandle(GetCurrentProcess(), GetStdHandle(STD_OUTPUT_HANDLE), GetCurrentProcess(), &si->hStdOutput, 0, true, DUPLICATE_SAME_ACCESS)) ***REMOVED***
...
***REMOVED***
  ***REMOVED***
  if (! si->hStdError)  ***REMOVED***
    if (! DuplicateHandle(GetCurrentProcess(), GetStdHandle(STD_ERROR_HANDLE), GetCurrentProcess(), &si->hStdError, 0, true, DUPLICATE_SAME_ACCESS)) ***REMOVED***
...
void close_output_handles(STARTUPINFO *si) ***REMOVED***
...
/*
  ...
  Returns:  0 on success.
           ...
           -1 on fatal error.
*/
static int try_read(logger_t *logger, void *address, unsigned long bufsize, unsigned long *in, int *complained) ***REMOVED***
  int ret = 1;
  unsigned long error;
  for (int tries = 0; tries < 5; tries++) ***REMOVED***
...
/*
  ...
  Returns:  0 on success.
           ...
           -1 on fatal error.
*/
static int try_write(logger_t *logger, void *address, unsigned long bufsize, unsigned long *out, int *complained) ***REMOVED***
  int ret = 1;
  unsigned long error;
  for (int tries = 0; tries < 5; tries++) ***REMOVED***
...
unsigned long WINAPI log_and_rotate(void *arg) ***REMOVED***
...
***REMOVED***
    ...
    if (*logger->rotate_online == NSSM_ROTATE_ONLINE_ASAP || (logger->size && size + (__int64) in >= logger->size)) ***REMOVED***
      /* Look for newline. */
      unsigned long i;
      for (i = 0; i < in; i++) ***REMOVED***
        if (buffer[i] == '\n') ***REMOVED***
      ...
      ***REMOVED***
          ...
          if (! logger->write_handle) ***REMOVED***
            error = GetLastError();
