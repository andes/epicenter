...
/* Open Policy object. */
int open_lsa_policy(LSA_HANDLE *policy) ***REMOVED***
...
int username_sid(const TCHAR *username, SID **sid, LSA_HANDLE *policy) ***REMOVED***
  LSA_HANDLE handle;
  if (! policy) ***REMOVED***
    policy = &handle;
  ...
  */
  ...
  if (_tcsnicmp(_T(".\\"), username, 2)) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
  ...
  if (translated_sid->Use != SidTypeUser && translated_sid->Use != SidTypeWellKnownGroup) ***REMOVED***
    LsaFreeMemory(translated_domains);
    LsaFreeMemory(translated_sid);
  ...
  ***REMOVED***
  ...
  if (! trust || ! IsValidSid(trust->Sid)) ***REMOVED***
    LsaFreeMemory(translated_domains);
    LsaFreeMemory(translated_sid);
    print_message(stderr, NSSM_GUI_INVALID_USERNAME, username);
    return 7;
  ...
  ***REMOVED***
  ...
  unsigned long error;
  if (! InitializeSid(*sid, GetSidIdentifierAuthority(trust->Sid), *n + 1)) ***REMOVED***
    error = GetLastError();
  ...
  ***REMOVED***
  ...
  for (unsigned char i = 0; i <= *n; i++) ***REMOVED***
  ...
  ***REMOVED***
  ...
  int ret = 0;
  if (translated_sid->Use == SidTypeWellKnownGroup && ! well_known_sid(*sid)) ***REMOVED***
    print_message(stderr, NSSM_GUI_INVALID_USERNAME, username);
    ret = 10;
  ***REMOVED***
...
int username_sid(const TCHAR *username, SID **sid) ***REMOVED***
  return username_sid(username, sid, 0);
***REMOVED***

int canonicalise_username(const TCHAR *username, TCHAR **canon) ***REMOVED***
  LSA_HANDLE policy;
  if (open_lsa_policy(&policy)) return 1;

  SID *sid;
  if (username_sid(username, &sid, &policy)) return 2;
  PSID sids = ***REMOVED*** sid ***REMOVED***;
...
int username_equiv(const TCHAR *a, const TCHAR *b) ***REMOVED***
  SID *sid_a, *sid_b;
  if (username_sid(a, &sid_a)) return 0;

  if (username_sid(b, &sid_b)) ***REMOVED***
    FreeSid(sid_a);
    return 0;
  ***REMOVED***
...
int is_localsystem(const TCHAR *username) ***REMOVED***
...
*/
const TCHAR *well_known_sid(SID *sid) ***REMOVED***
  if (! imports.IsWellKnownSid) return 0;
...
const TCHAR *well_known_username(const TCHAR *username) ***REMOVED***
...
int grant_logon_as_service(const TCHAR *username) ***REMOVED***
  ...
  SID *sid;
  if (username_sid(username, &sid, &policy)) ***REMOVED***
    LsaClose(policy);
    return 2;
  ***REMOVED***
...
***REMOVED***
  ***REMOVED***
  ...
  for (unsigned long i = 0; i < count; i++) ***REMOVED***
